...
def test_shade_setpos_parameters(base_profile: dict):
    shade = next(n for n in base_profile["nodedefs"] if n["id"] == "shadeid")
    setpos = next(c for c in shade["cmds"]["accepts"] if c["id"] == "SETPOS")
    param_ids = [p["id"] for p in setpos["parameters"]]
    assert param_ids == ["SETPRIM", "SETSECO", "SETTILT"]


def test_rts_movepct_default_parameter(base_profile: dict):
    rts = next(n for n in base_profile["nodedefs"] if n["id"] == "shadertsid")
